...
# os.system("title 内网穿透-IOT-80")#修改命令行的显示标题
logging.basicConfig(level=logging.DEBUG, format='[%(asctime)s] [%(levelname)s] [%(name)s] %(message)s',
                    datefmt='%Y/%m/%d %H:%M:%S')
logger = logging.getLogger('%s' % 'client')
logger.info('python-ngrok v1.42')
# host = 'server.ngrok.cc' # Ngrok服务器地址
# host = '172.17.1.151' # Ngrok服务器地址
host = '120.79.34.76'  # Ngrok服务器地址
# host = 'mybeebe.top' # Ngrok服务器地址
port = 4443  # 端口
bufsize = 1024  # 吞吐量
